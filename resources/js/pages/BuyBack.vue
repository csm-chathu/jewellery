      ...
      <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr v-for="b in buybacks.data" :key="b.id" class="hover:bg-gray-50">
            ...
            <td class="table-td">
              <span v-if="b.proof_images?.length" class="badge bg-blue-100 text-blue-700 text-xs">
                {{ b.proof_images.length }} photo{{ b.proof_images.length > 1 ? 's' : '' }}
              </span>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div class="p-6 space-y-6">
            ...
            <div class="border rounded-xl p-4 space-y-3 bg-gold-50/40">
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <template v-if="form.payment_method === 'cheque'">
                  ...
                  <div>
                    <label class="form-label">Cheque Bank Name *</label>
            ...
            <div class="border rounded-xl p-4 space-y-3">
              <p class="text-sm font-semibold text-gray-700">Proof Photos</p>
              <SmartImageUploader
                ref="proofUploader"
                v-model="proofImages"
                label="Upload proof images (multiple)"
                :multiple="true"
                :max-items="10"
                folder="jewellery/buybacks"
                :tags="['buyback','proof']"
              />
            </div>
...
const blankForm = () => ({
  ...
  kyc_verified: false, status: 'pending', notes: '', create_scrap: true, proof_images: [],
})
...
function openModal(b) {
  ...
  if (b) {
    ...
    proofImages.value = b.proof_images || []
  } else {
...
async function save() {
  ...
  try {
    if (proofUploader.value) {
      await proofUploader.value.uploadPending()
    }
